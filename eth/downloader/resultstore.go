...
func (r *resultStore) AddFetch(header *types.Header, fastSync bool, dhtSync bool) (stale, throttled bool, item *fetchResult, err error) {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	if item == nil {
		item = newFetchResult(header, fastSync, dhtSync)
		r.items[index] = item
	}
	return stale, throttled, item, err
}
